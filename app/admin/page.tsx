...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f1eb] p-4 md:p-8 text-black">
      <h1 className="text-2xl font-black mb-6">DERBAAG ADMIN - แก้ไขได้จริง</h1>
      <div className="bg-white rounded-2xl p-5 shadow-sm border mb-8">
        <p className="font-bold mb-3">+ {editingId? `แก้ไข ID ${editingId}` : "เพิ่มสินค้าใหม่"}</p>
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <select value={form.category} onChange={e=>setForm({...form, category: e.target.value})} className="bg-[#f2f0eb] rounded-full px-4 py-2.5 w-36 outline-none">
            <option>เสื้อ</option><option>หมวก</option><option>ผ้าฟลีซ</option><option>ผ้าเช็ดหน้า</option><option>กางเกง</option>
          </select>
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[#f7f5f2] text-left"><tr><th className="p-3">รูป</th><th className="p-3">ID</th><th className="p-3">ชื่อ</th><th className="p-3">ราคา</th><th className="p-3">สถานะ</th><th className="p-3">สต็อก</th><th className="p-3">จัดการ</th></tr></thead>
            <tbody>
              {products.map(p=>(
                <tr key={p.id} className="border-t">
                  <td className="p-2"><img src={getImage(p)} alt="" className="w-12 h-12 rounded-lg object-cover bg-zinc-100" onError={(e)=>{(e.target as any).src="https://placehold.co/100x100?text=No+Image"}} /></td>
                  <td className="p-3">{p.id}</td><td className="p-3 font-bold">{p.name}</td><td className="p-3">฿{p.price}</td>
                  <td className="p-3"><span className="bg-black text-white text- px-2.5 py-1 rounded-full">{p.status}</span></td>
                  <td className="p-3">{p.stock}</td>
                  <td className="p-3 flex gap-2"><button onClick={()=>handleEdit(p)} className="bg-black text-white rounded-full px-3 py-1 text-xs">แก้</button><button onClick={()=>handleDelete(p.id)} className="bg-[#b5452a] text-white rounded-full px-3 py-1 text-xs">ลบ</button></td>
                </tr>
              ))}
            </tbody>
          </table>
