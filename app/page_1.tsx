...
interface Product {
  ...
  // preorder only
  preorderEndsAt?: number; // timestamp ms
  daysLeft: number;
  ordered: number;
  stock?: number;
  image: string;
...
const toFuture = (d: number, h: number, m: number, s: number) =>
  nowBase + ((d * 24 + h) * 3600 + m * 60 + s) * 1000;

// Mock Data with 4 statuses
const PRODUCTS: Product[] = [
  {
    id: '1',
    name: 'เสื้อเดินป่า Ultralight',
    nameEn: 'Trail Ultralight Tee',
    price: 590,
    category: 'เสื้อ',
    status: 'preorder',
    preorderEndsAt: toFuture(5, 14, 32, 10),
    daysLeft: 5,
    ordered: 128,
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1571513800374-df1bbe650e56?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=600&h=700&fit=crop',
    ],
    material: 'Polyester Recycled 100% • น้ำหนัก 120g • UPF 50+',
    colors: [{ name: 'Forest', hex: '#1E3A2E' }, { name: 'Sand', hex: '#D9C5A0' }, { name: 'Clay', hex: '#C86B3C' }],
  },
  {
    id: '2',
    name: 'หมวกบักเก็ตเดินป่า',
    nameEn: 'Bucket Trail Hat',
    price: 390,
    category: 'หมวก',
    status: 'preorder',
    preorderEndsAt: toFuture(3, 2, 15, 40),
    daysLeft: 3,
    ordered: 86,
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1533827432537-70133748f5c8?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1556306535-38febf6782e7?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1576871337632-b9aef4c17ab9?w=600&h=700&fit=crop',
    ],
    material: 'Nylon Ripstop • กันน้ำ DWR • น้ำหนัก 65g',
    colors: [{ name: 'Moss', hex: '#4A5D4E' }, { name: 'Sand', hex: '#E8DCC6' }],
  },
  {
    id: '3',
    name: 'ผ้าบัฟลายป่า',
    ...
    category: 'ผ้าพัฟ',
    status: 'preorder',
    preorderEndsAt: toFuture(7, 8, 5, 22),
    daysLeft: 7,
    ordered: 212,
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1611923134232-9d6d660e7d32?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1600185365483-26d7a4cc7519?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1511556820780-d912e42b4980?w=600&h=700&fit=crop',
    ],
    material: 'Microfiber • ยืด 4 ทิศทาง • แห้งไว',
    colors: [{ name: 'Forest Print', hex: '#1E3A2E' }, { name: 'Sunset', hex: '#FF6B2C' }],
  },
  {
    id: '4',
    name: 'ผ้าเช็ดหน้าวินเทจ',
    nameEn: 'Vintage Handkerchief',
    price: 180,
    category: 'ผ้าเช็ดหน้า',
    status: 'instock',
    daysLeft: 0,
    ordered: 94,
    stock: 24,
    image: 'https://images.unsplash.com/photo-1590732596287-05ff25b6588f?w=600&h=700&fit=crop',
    gallery: [
      'https://images.unsplash.com/photo-1590732596287-05ff25b6588f?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1584302052179-2e90841dad6a?w=600&h=700&fit=crop',
    ...
    material: 'Organic Cotton • พิมพ์ลายธรรมชาติ • 50x50cm',
    colors: [{ name: 'Ecru', hex: '#F5F1EB' }, { name: 'Olive', hex: '#6B7A5A' }],
  },
  {
    id: '5',
    name: 'เสื้อแขนยาวกันแดด',
    nameEn: 'Sun Block Longsleeve',
    price: 690,
    category: 'เสื้อ',
    status: 'instock',
    daysLeft: 0,
    ordered: 156,
    stock: 12,
    image: 'https://images.unsplash.com/photo-1620799139652-715239583449?w=600&h=700&fit=crop',
    gallery: [
      'https://images.unsplash.com/photo-1620799139652-715239583449?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1618354691321-e851c56960d1?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=600&h=700&fit=crop',
    ],
    material: 'Merino Blend • กันแดด UPF 50+ • ระบายอากาศ',
    colors: [{ name: 'Stone', hex: '#9A8C7D' }, { name: 'Forest', hex: '#1E3A2E' }],
  },
  {
    id: '6',
    name: 'หมวกแก๊ปผ้าฝ้าย',
    nameEn: 'Cotton Trail Cap',
    price: 350,
    category: 'หมวก',
    status: 'comingsoon',
    daysLeft: 0,
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1533827432537-70133748f5c8?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1514327605112-b887c0e61c0a?w=600&h=700&fit=crop',
    ],
    material: 'Cotton Canvas • สายปรับหลัง • ปักโลโก้ DERBAAG',
    colors: [{ name: 'Khaki', hex: '#C2B280' }, { name: 'Black', hex: '#111111' }],
  },
  {
    id: '7',
    name: 'กางเกงเดินป่า 3Season',
    nameEn: '3Season Trek Pants',
    price: 890,
    category: 'เสื้อ',
    status: 'closed',
    daysLeft: 0,
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1506629082955-511b1aa562c8?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1517668808822-9ebb02f2a0e6?w=600&h=700&fit=crop',
      'https://images.unsplash.com/photo-1551028719-00167b16eac5?w=600&h=700&fit=crop',
    ],
    material: 'Nylon Stretch • กันน้ำ DWR • น้ำหนัก 280g',
    colors: [{ name: 'Olive', hex: '#4A5D4E' }, { name: 'Charcoal', hex: '#333333' }],
  },
];
...
function PreorderStepper({ activeStep }: { activeStep: number }) {
  ...
  const progress = activeStep > 0 ? ((activeStep - 1) / (steps.length - 1)) * 100 : 0;
  return (
      ...
      <div className="bg-white border-y lg:border border-black/[0.06] lg:rounded-[24px] shadow-[0_16px_48px_rgba(30,58,46,0.12)] px-6 lg:px-9 py-7 lg:py-8">
        <div className="flex items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className="w-11 h-11 rounded-full bg-[#1E3A2E] text-white grid place-items-center shadow-sm">
              <Package className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-[17px] font-bold tracking-[0.06em] leading-none">ขั้นตอนการ Pre-order</h3>
              <p className="text-[13px] text-black/50 mt-1.5 hidden sm:block font-medium">ผลิตตามจำนวนจริง • ลดขยะ • ส่งมอบตามรอบ</p>
            </div>
            <span className="hidden sm:inline-flex ml-3 px-3.5 py-1.5 rounded-full bg-[#FFF0E8] text-[#FF6B2C] text-[12px] font-bold border border-[#FF6B2C]/20 tracking-wide">MADE TO ORDER</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-[13px] font-bold px-3 py-1.5 rounded-full bg-[#F5F1EB] border border-black/5">{activeStep}/4</span>
          </div>
        </div>

        <div className="relative">
          {/* Base line */}
          <div className="absolute top-[34px] left-[12%] right-[12%] h-[4px] bg-[#EDE7DE] rounded-full hidden sm:block" />
          {/* Progress line */}
          <div className="absolute top-[34px] left-[12%] h-[4px] bg-[#1E3A2E] rounded-full transition-all duration-700 hidden sm:block" style={{ width: `calc(${progress}% * 0.76)` }} />
          {/* Orange glow for current */}
          {activeStep > 1 && <div className="absolute top-[34px] h-[4px] bg-[#FF6B2C] rounded-full hidden sm:block transition-all duration-700" style={{ left: `calc(12% + ${((activeStep - 2) / 3) * 76}% )`, width: `calc(76% / 3)` }} />}
            ...
            {steps.map((s) => {
              ...
              return (
                <div key={s.id} className="flex flex-col items-center text-center">
                  <div
                    className={`w-[64px] h-[64px] sm:w-[72px] sm:h-[72px] rounded-full grid place-items-center border-2 transition-all duration-300
                      ${state === 'completed' ? 'bg-[#1E3A2E] border-[#1E3A2E] text-white shadow-md' : ''}
                      ${state === 'current' ? 'bg-[#FF6B2C] border-[#FF6B2C] text-white shadow-[0_10px_30px_rgba(255,107,44,0.40)] scale-[1.08] ring-[6px] ring-[#FF6B2C]/15' : ''}
                      ${state === 'upcoming' ? 'bg-[#F5F1EB] border-black/10 text-black/25' : ''}
                    `}
                  >
                    {state === 'completed' ? <Check className="w-7 h-7" /> : <s.Icon className="w-7 h-7" />}
                  </div>
                  <p className={`mt-4 text-[14px] sm:text-[15px] leading-tight font-bold ${state === 'current' ? 'text-[#FF6B2C]' : state === 'completed' ? 'text-[#1E3A2E]' : 'text-black/40'}`}>{s.label}</p>
                  <p className={`mt-1 text-[12px] sm:text-[13px] leading-tight font-medium ${state === 'upcoming' ? 'text-black/35' : 'text-black/60'}`}>{s.desc}</p>
                  {state === 'current' && <span className="mt-2.5 hidden sm:inline-flex w-2 h-2 rounded-full bg-[#FF6B2C] animate-pulse" />}
                </div>
              );
            })}
          </div>
...
export default function App() {
  ...
  // Live ticking for preorder countdowns (global)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#F5F1EB] text-[#1E3A2E] antialiased selection:bg-[#FF6B2C] selection:text-white text-[18px] leading-relaxed" style={{ fontFamily: "Tahoma, sans-serif", fontSize: '18px' }}>
      <style>{`
        * { font-family: Tahoma, sans-serif !important; }
        ...
        /* FIX: Prevent wrapping of Pre-order รอบที่ 08 and countdown badges */
        .preorder-nowrap {
        ...
        /* Ensure all pill badges don't wrap on mobile */
        .rounded-full {
          white-space: nowrap;
        }
        /* Specific fix for headings containing Pre-order รอบที่ 08 */
        h1.preorder-title-fix {
          white-space: nowrap !important;
          display: inline-flex !important;
          gap: 6px !important;
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-black/[0.06] shadow-[0_2px_20px_rgba(30,58,46,0.06)]">
        <div className="max-w-[1360px] mx-auto px-6 lg:px-10">
          {/* Top row */}
          <div className="h-[72px] lg:h-[80px] flex items-center justify-between gap-5">
            <div className="flex items-center gap-7">
              <button onClick={() => setCurrentPage('products')} className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-[#1E3A2E] text-[#F5F1EB] grid place-items-center font-bold text-[13px] tracking-widest">DB</div>
                <span className="font-bold tracking-[0.18em] text-[20px]">DERBAAG</span>
              </button>
              <div className="hidden lg:flex items-center gap-2.5 text-[13px] text-black/50 font-medium whitespace-nowrap">
                <span className="w-2 h-2 rounded-full bg-[#FF6B2C] animate-pulse shrink-0" />
                <span className="preorder-nowrap">PRE-ORDER รอบที่ 08</span>
                <span className="whitespace-nowrap">• ส่งมอบ ส.ค. 2026 • ผลิตตามออเดอร์</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="hidden md:flex items-center gap-2.5 h-12 px-4 rounded-full bg-[#F5F1EB] border border-black/5 text-[15px]">
                <Search className="w-5 h-5 opacity-40" />
                <input placeholder="ค้นหา..." className="bg-transparent outline-none w-[140px] lg:w-[200px] placeholder:text-black/30 text-[15px]" />
              </div>
              <button className="w-12 h-12 rounded-full bg-[#F5F1EB] border border-black/5 grid place-items-center hover:bg-black/[0.04] transition">
                <Heart className="w-5 h-5" />
              </button>
              <button onClick={() => setCurrentPage('checkout')} className="relative w-12 h-12 rounded-full bg-[#F5F1EB] border border-black/5 grid place-items-center hover:bg-black/[0.04] transition">
                <ShoppingBag className="w-5 h-5" />
                <span className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-[#FF6B2C] text-white text-[11px] grid place-items-center font-bold shadow-sm">{cartCount}</span>
              </button>
              <div className="w-12 h-12 rounded-full bg-[#1E3A2E] grid place-items-center text-white shadow-sm"><User className="w-5 h-5" /></div>
            </div>
          </div>
          {/* Main Nav Tabs - BIG */}
          <div className="h-[56px] lg:h-[64px] flex items-center overflow-auto scrollbar-none -mx-6 px-6 lg:mx-0 lg:px-0 border-t border-black/[0.06]">
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative h-[56px] px-6 whitespace-nowrap text-[17px] transition-all flex items-center rounded-full lg:rounded-none
                      ${active ? 'font-bold text-[#1E3A2E] bg-[#F5F1EB] lg:bg-transparent' : 'font-semibold text-black/50 hover:text-black/80 hover:bg-black/[0.03] lg:hover:bg-transparent'}
                    `}
                  >
                    {item.label}
                    {active && <span className="absolute bottom-0 left-4 right-4 h-[4px] rounded-full bg-[#FF6B2C]" />}
                  </button>
                );
              })}
            </nav>
            <div className="ml-auto hidden lg:flex items-center gap-4 pl-8 border-l border-black/5">
              <span className="text-[12px] tracking-[0.12em] text-black/40 font-bold">PRE-ORDER FLOW</span>
              <div className="flex items-center gap-1.5">
                {[1, 2, 3, 4].map((n) => {
                  const step = getActiveStep(currentPage);
                  const done = step >= n;
                  return <div key={n} className={`h-2 rounded-full transition-all ${done ? (step === n ? 'bg-[#FF6B2C] w-12' : 'bg-[#1E3A2E] w-8') : 'bg-black/10 w-8'}`} />;
                })}
              </div>
            </div>
          </div>
